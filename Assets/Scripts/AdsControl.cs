...
public class AdsControl : MonoBehaviour
{
    ...
    public Action actionCloseRW;
    //public LimitAds limitAds;
    ...
    public void ShowInterstitialAdCallBack(Action callBack)
    {
        actionCloseInter = null;
        actionCloseInter = callBack;
        if (interstitialAd != null && interstitialAd.IsLoaded() /*&& limitAds.canShowAds*/)
        {
            interstitialAd.Show();
        }
        else
    ...
    public void RequestAndLoadRewardedAd()
    {
        ...
        rewardedAd.OnAdClosed += (sender, args) =>
        {
            ...
            RequestAndLoadRewardedAd();
            OnAdClosedEvent.Invoke();
            actionCloseRW?.Invoke();
        };
        rewardedAd.OnUserEarnedReward += (sender, args) =>
        {

            // PrintStatus("User earned Reward ad reward: " + args.Amount);
            //if (currentType == REWARD_TYPE.BOMB)
            //{
            //    GameManager.instance.uiManager.gamePanel.UseBombCallback();
            //}
            //else if (currentType == REWARD_TYPE.HERO)
            //{
            //    GameManager.instance.uiManager.gamePanel.UseHeroCallback();
            //}
            //else if (currentType == REWARD_TYPE.MULTI_COIN)
            //{

            //    if (GameManager.instance.currentState == GameManager.STATE.GAME_FINISH)
            //        GameManager.instance.uiManager.winPanel.RWCallBack();
            //    else if (GameManager.instance.currentState == GameManager.STATE.GAME_FAIL)
            //        GameManager.instance.uiManager.defeatPanel.RWCallBack();
            //}
    ...
    public void ShowRewardedAdCallBack(Action callback)
    {
        actionCloseRW = null;
        actionCloseRW = callback;
        if (rewardedAd != null)
        {
