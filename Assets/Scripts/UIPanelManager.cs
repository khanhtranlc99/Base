...
public class UIPanelManager : MonoBehaviour
{
		...
		if (_currentPanelObject != null)
		{
			...
			switch (panelName)
			{
				case "LevelPanel":
					...
					if (_currentPanelObject.GetComponent<UIPanelBase>()._PanelName == "PlayPanel")
					{
				...
				case "PlayPanel":
					...
					if (_currentPanelObject.GetComponent<UIPanelBase>()._PanelName == "WinPanel" || _currentPanelObject.GetComponent<UIPanelBase>()._PanelName == "SkipPanel")
					{
						HideUIScreen(_currentPanelObject.GetComponent<UIPanelBase>(), true, true, Tween.TweenStyle.EaseOut, null);
						_lastPanelObject = _currentPanelObject;
						Destroy(_panelPairs["PlayPanel"]);
						_panelPairs.Remove(panelName);
						Destroy(_lastPanelObject, 0.5f);
			...
			case "GuidePanel":
				{
					...
					_currentPanelObject = _panelPairs["PlayPanel"];
					if (_lastPanelObject != null && _currentPanelObject != null)
						_currentPanelObject.GetComponent<UIPanelBase>()._LastPanel = _lastPanelObject.GetComponent<UIPanelBase>()._PanelName;
					_currentPanelObject.GetComponent<UIPanelBase>()._PanelName = "PlayPanel";
				}
				break;
			case "MenuPanel":
                {
	...
	IEnumerator ShowWinPanelIE()
	{
		...
		AdsControl.Instance.ShowInterstitialAdCallBack(delegate { });
	}
